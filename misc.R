...
##### Write BTB output for each PDID
write_btb = function(){
  mainDir = '~/lustre_mt22/CN_methods/revision_2204_v2/'  
  # Set outdir
  outdir = file.path(mainDir,'BTB_output')
  for(tumourType in unique(projMani$TumourType)){
    if(tumourType %in% c('NB','RCC','ATRT','Wilms','Ewings')){
      for(current_PDID in unique(projMani$PDID[projMani$TumourType == tumourType])){
        ...
        dna.data = processBTB(btb.fp,minSegLen=0,subCl.minSegLen = subCl.minSegLen,PDID=current_PDID,tgtChrs=chrToPlot,removeBalancedSegs=F,longFormat = F,keepClonalSegs = T,method = 'config')  
        dna.data = as.data.frame(mcols(dna.data))
        
        #dna.data$matNum = ifelse(is.na(dna.data$matNum),dna.data$totCN * as.numeric(dna.data$tumFrac),dna.data$matNum)
        #dna.data$patNum = ifelse(is.na(dna.data$patNum),dna.data$totCN - dna.data$matNum,dna.data$patNum)
        out = dna.data[,c("chr","Start","Stop","matNum","patNum","clonalType" )]
        out$subcloneCN = ifelse(out$clonalType == 'sub',TRUE,FALSE)
        out=out[,-c(6)]
        out$PDID = current_PDID
        write.csv(out,paste0(outdir,'/',current_PDID,'_btbCN_2.csv'),col.names = T)
      }
    }
  }
...
processBTB = function(btb.fp,PDID,tgtChrs=c(1:23,'X'),longFormat=T,removeBalancedSegs = FALSE,method = c('totalCN','allelicRatio','config'),minSegLen=1e6,subCl.minSegLen=1e7,keepClonalSegs=F){
