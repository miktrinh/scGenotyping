...
for(tumourType in unique(projMani$TumourType)){
  if(tumourType %in% c('NB','RCC','Wilms','ATRT','Ewings')){
    ...
    # Make output dir
    if(normREF){
      outDir = file.path(mainDir,'../revision_2204_v2/inferCNV_CK_plots/normREF')  
    }else{
      outDir = file.path(mainDir,'../revision_2204_v2/inferCNV_CK_plots/noNormREF')  
    }
    ...
    for(current_PDID in unique(projMani$PDID[projMani$TumourType == tumourType])){
      ...
      saveFig(file.path(outDir,paste0('FigS2_',tumourType,'_CK.inferCNV_',current_PDID,'_',subCl.minSegLen)),plotFun,width = 2.0,height = 3.2,res=500)  
    }
  }
}
